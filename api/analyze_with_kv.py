...
# Vercel KV imports (if available)
try:
    from vercel_kv import kv
    KV_AVAILABLE = True
except ImportError:
    KV_AVAILABLE = False
    print("Vercel KV not available, using memory cache")
...
class MenuAnalyzer:
    ...
    def get_cache_key(self, date_str: str) -> str:
        ...
        return f"menu_analyzer:{hashlib.md5(key_string.encode()).hexdigest()}"

    def get_cached_result(self, cache_key: str) -> Optional[Dict[str, List[Tuple[str, int, str, str]]]]:
        """Check Vercel KV cache or memory cache"""
        try:
            if KV_AVAILABLE:
                # Use Vercel KV for persistent caching
                ...
                if cached_data:
                    ...
                    if time.time() - cache_time < 3600:  # 1 hour cache
                        if self.debug:
                            print(f"Using Vercel KV cached results for {self.campus_key}")
                ...
                if cached_data:
                    ...
                    if time.time() - cache_time < 3600:  # 1 hour cache
                        if self.debug:
                            print(f"Using memory cached results for {self.campus_key}")
    ...
    def save_cached_result(self, cache_key: str, results: Dict[str, List[Tuple[str, int, str, str]]]):
        """Save results to Vercel KV or memory cache"""
        cache_data = {
            'results': results,
            'timestamp': time.time()
        }
        ...
        try:
            if KV_AVAILABLE:
                # Use Vercel KV for persistent caching
                kv.set(cache_key, json.dumps(cache_data), ex=3600)  # 1 hour expiry
                if self.debug:
                    print(f"Cached results in Vercel KV for {self.campus_key}")
            else:
                # Fallback to memory cache
                if not hasattr(self, '_memory_cache'):
                    self._memory_cache = {}
                ...
                if self.debug:
                    print(f"Cached results in memory for {self.campus_key}")
        except Exception as e:
            if self.debug:
    ...
    def analyze_menu_with_gemini(self, daily_menu: Dict[str, Dict[str, str]]) -> Dict[str, List[Tuple[str, int, str, str]]]:
        ...
        max_retries = 3  # Reduced for serverless
        base_delay = 1   # Reduced for serverless
    ...
    def run_analysis(self) -> Dict[str, List[Tuple[str, int, str, str]]]:
        ...
        cache_key = self.get_cache_key(today_str_key)
        ...
        # Save to cache
        self.save_cached_result(cache_key, final_results)
        ...
        if self.path == '/api/analyze':
            try:
                ...
                if vegan and vegetarian:
                    ...
                    self.end_headers()
                    self.wfile.write(json.dumps({"error": "Cannot be both vegan and vegetarian"}).encode())
                    return
